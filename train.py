...
def joint_loss(pred, mask):
    ...
    mask = F.one_hot(mask, num_classes=2).permute(0, 3, 1, 2)
    weit = 1 + 5*torch.abs(F.avg_pool2d(mask.float(), kernel_size=31, stride=1, padding=15) - mask)
    wbce = F.binary_cross_entropy_with_logits(pred, mask.float(), reduction='none')
...
def main_tr(args, crossVal):
    ...
    # load the model
    model = net.PHFSeg(args.classes, aux=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--savedir', default='./results_PHFSeg_crossVal', help='Directory to save the results')
    ...
    parser.add_argument('--data_name', default='P20', help='dataset name, P1100 or P20')
