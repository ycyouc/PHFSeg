...
from module.TripletAttention import TripletAttention
from module.SpectrumTransformer import SpectrumTransformer
...
class PHFSeg(nn.Module):
    def __init__(self, classes=2, P1=2, P2=3, P3=8, P4=6, aux=True):
        ...
        self.long2 = SpectrumTransformer(8, 24, stride=2)
        self.down2_1x1 = nn.Conv2d(8, 24, 1, stride=2)
        self.down2 = TripletAttention(8, 24, stride=2)
        self.level2 = nn.ModuleList()
        self.level2_long = nn.ModuleList()
        for i in range(0, P2):
            self.level2.append(TripletAttention(24, 24))
        for i in range(0, self.D2):
            self.level2_long.append(SpectrumTransformer(24, 24, stride=1))

        self.cat2 = nn.Sequential(
                    nn.Conv2d(48, 48, 1, stride=1, padding=0, groups=1, bias=False),
                    nn.BatchNorm2d(48))

        self.long3 = SpectrumTransformer(24, 32, stride=2)
        self.down3 = TripletAttention(24, 32, stride=2)
        self.down3_1x1 = nn.Conv2d(24, 32, 1, stride=2)
        self.level3 = nn.ModuleList()
        self.level3_long = nn.ModuleList()
        for i in range(0, P3):
            self.level3.append(TripletAttention(32, 32))
        for i in range(0, self.D3):
            self.level3_long.append(SpectrumTransformer(32, 32, stride=1))

        self.cat3 = nn.Sequential(
                    nn.Conv2d(64, 64, 1, stride=1, padding=0, groups=1, bias=False),
                    nn.BatchNorm2d(64))

        self.long4 = SpectrumTransformer(32, 64, stride=2)
        self.down4 = TripletAttention(32, 64, stride=2)
        self.down4_1x1 = nn.Conv2d(32, 64, 1, stride=2)
        self.level4 = nn.ModuleList()
        self.level4_long = nn.ModuleList()
        for i in range(0, P4):
            self.level4.append(TripletAttention(64, 64))
        for i in range(0, self.D4):
            self.level4_long.append(SpectrumTransformer(64, 64, stride=1))
